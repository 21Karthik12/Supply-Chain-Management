...
def handle_json_message(data):
    ...
    server.emit('json_response', {'response': 'Received your JSON message'}, namespace=NAMESPACE)
...
def handle_create_node():
    ...
    try:
        ...
        if request_data:
            ...
            subprocess.Popen(['python', '.\sensors\sensor.py', str(node_id), node_type, node_module])
            ids.add(node_id)
            sensor_id += 1
